import { IsArray, IsOptional, IsString, ValidateNested, IsEmail, IsEnum } from 'class-validator';
...
export class BulkUpdateOrderDto {
  ...
  @ApiPropertyOptional({ description: 'New status for all orders', enum: OrderStatus })
  @IsOptional()
  @IsEnum(OrderStatus)
  status?: OrderStatus;

  @ApiPropertyOptional({ description: 'New payment status for all orders', enum: PaymentStatus })
  @IsOptional()
